...
namespace i2c{
	...
	********************/
	...
	DeviceState I2CDevice::Initialize(uint8_t device_addr,uint8_t device_mode){
		...
		SetAddrfd(device_fd,device_addr);
		return device_state;
	}
	...
	********************/
	...
	DeviceState AQM0802::Initialize(uint8_t device_addr,uint8_t device_mode){
		...
		wait_usec(100000);
		// Function set = 0x38
		if(Writei2c(device_fd,CMD,0x38)!=I2C_OK){return device_state;}
		// Wait 26.3us
		wait_usec(1000);
		// Function set = 0x39
		if(Writei2c(device_fd,CMD,0x39)!=I2C_OK){return device_state;}
		// Wait 26.3us
		wait_usec(1000);
		// Internal OSC frequency = 0x14
		if(Writei2c(device_fd,CMD,0x14)!=I2C_OK){return device_state;}
		// Wait 26.3us
		wait_usec(1000);
		// Contrast set = 0x70
		if(Writei2c(device_fd,CMD,0x70)!=I2C_OK){return device_state;}
		// Wait 26.3us
		wait_usec(1000);
		// Power/ICON/Contrast control = 0x56
		if(Writei2c(device_fd,CMD,0x56)!=I2C_OK){return device_state;}
		// Wait 26.3us
		wait_usec(1000);
		// Follower control = 0x6C
		if(Writei2c(device_fd,CMD,0x6c)!=I2C_OK){return device_state;}
		// Wait 200ms
		wait_usec(200000);
		// Function set = 0x38
		if(Writei2c(device_fd,CMD,0x38)!=I2C_OK){return device_state;}
		...
		if(Writei2c(device_fd,CMD,0x0c)!=I2C_OK){return device_state;}
		// Wait 26.3us
		wait_usec(1000);
		// Clear Display = 0x01
		if(Writei2c(device_fd,CMD,0x01)!=I2C_OK){return device_state;}
		// Wait 1.08ms
		wait_usec(2000);
	...
	DeviceState AQM0802::SetLcdData(const uint8_t *buffer,uint32_t length){
		...
		//write data with i2c
		if(Writei2c(device_fd,DAT,buffer,length)==I2C_OK){
			device_state=FUNC_OK;
		}
		else{
			CheckI2CError();
	...
	DeviceState AQM0802::SetLocate(uint32_t x,uint32_t y){
		...
		//write data with i2c
		if(Writei2c(device_fd,CMD,(0x80 + y*0x40 + x))==I2C_OK){
			device_state=FUNC_OK;
		}else{
			CheckI2CError();
		}
	...
	DeviceState AQM0802::ClearDisp(){
		...
		//Clear Display = 0x01
		if(Writei2c(device_fd,CMD,0x01)!=I2C_OK){
			return CheckI2CError();
		}
		
		wait_usec(2000);// Wait 1.08ms
		//Return Home = 0x03
		if(Writei2c(device_fd,CMD,0x03)!=I2C_OK){
			return CheckI2CError();
		}
	...
	********************/
	...
	void LPS331AP::SoftwareReset(){
		Writei2c(device_fd,0x21,0x84);
		wait_usec(100000);
		Writei2c(device_fd,0x21,0x00);
	}
	...
	DeviceState LPS331AP::Initialize(uint8_t device_addr,uint8_t device_mode){
		...
		if(Readi2c(device_fd,0x0f,&buff,1)!=I2C_OK){
			return device_state;
		}
		...
		if(Writei2c(device_fd,0x20,0x00)!=I2C_OK){//PD
			return device_state;
		}
		wait_usec(1000);
		...
		if(Writei2c(device_fd,0x20,0x80+(device_mode<<4)+0x02)!=I2C_OK){//PU and set mode
			return device_state;
		}
		wait_usec(10);
	...
	DeviceState LPS331AP::ReadData(){
		...
		if(device_mode==LPS_ONESHOT){
			if(Writei2c(device_fd,0x21,0x01)!=I2C_OK){
				return CheckI2CError();
			}
			//check status_reg
			uint32_t i=0;
			...
			do{
				...
				wait_usec(10000);
				if(Readi2c(device_fd,0x27,&state,1)!=I2C_OK){
					return CheckI2CError();
				}
			}while((state & 0x03)==0x00);
		}

		if(Readi2c(device_fd,0x28+0x80,data,5)!=I2C_OK){
			return CheckI2CError();
		}
	...
	********************/
	...
	void ADT7410::SoftwareReset(){
		Writei2c(device_fd,0x2f,0x00,0);
		wait_usec(100000);
	}
	...
	DeviceState ADT7410::Initialize(uint8_t device_addr,uint8_t device_mode){
		...
		if(Writei2c(device_fd,0x03,0x80+(this->device_mode<<5))!=I2C_OK){
			return device_state;
		}
	...
	DeviceState ADT7410::ReadData(){
		...
		if(device_mode==ADT_ONESHOT){//one-shot
			state=0xa0;
			if(Writei2c(device_fd,0x03,&state,1)!=I2C_OK){
				return CheckI2CError();
			}
		//check status_reg
		wait_usec(250000);
		...
		do{
			...
			wait_usec(10000);
			if(Readi2c(device_fd,0x02,&state,1)!=I2C_OK){
				return CheckI2CError();
			}
		}while(state & 0x80);
	
		if(Readi2c(device_fd,0x00,buff,2)!=I2C_OK){
			return CheckI2CError();
		}
...
#define SHT21_READ_RH 0xf5
#define SHT21_READ_TMP 0xf3
	...
	DeviceState SHT21::ReadData(){
		uint8_t buff[3];
		Writei2c(device_fd,SHT21_READ_RH,buff,0);
//		if(Writei2c(device_fd,SHT21_READ_RH,buff,0)!=I2C_OK){
//			return CheckI2CError();
//		}
		wait_usec(100000);
		//buff[3]:dummy;
		if(Readi2c(device_fd,buff,3)!=I2C_OK){
			return CheckI2CError();
		}
		buff[1]&=0xfc;
		...
		Writei2c(SHT21_ADDR,SHT21_READ_TMP,buff,0);
//		if(Writei2c(SHT21_ADDR,SHT21_READ_TMP,buff,0)!=I2C_OK){
//			return CheckI2CError();
//		}
		wait_usec(100000);
		if(Readi2c(device_fd,buff,3)!=I2C_OK){
